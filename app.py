...
def v3():
    """
    Renders v3 model input and form results
    """
...
def retrieve_recommendations_for_model(question, model):
    ...
    We use an LRU cache to store results we process. If we see same
    question twice, we can retrieve cached results to serve them faster
    :param question: the input text to the model
    :param model: which model to use
    :return : a model's recommendation
    """
    if model == "v1":
...
def handle_text_request(request, template_name):
    ...
    posted question of for a POST request
